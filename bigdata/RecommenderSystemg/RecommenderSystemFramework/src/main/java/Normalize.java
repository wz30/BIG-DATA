...
public class Normalize {
    ...
    public static class NormalizeReducer extends Reducer<Text, Text, Text, Text> {
        ...
        public void reduce(Text key, Iterable<Text> values, Context context)
    	    ...
    	    //build hashmap
    //key: movieB
    //value, movie:relation/sum
    	    int sum = 0; //sum of relation
	    Map<String, Integer> map = new HashMap<String, Integer>();
	    while(values.iterator.hashNext()){
		String[] movieB_relation = values.iterator.next().toString().split(:);
	    ...
	    while(Map.entry<String,Integer> entry : map.entrySet()){
		    String outputKey = entry.getKey();
		    String outputValues = key.toString() + "=" + (double)entry.getValue()/sum;
		    context.write(new Text(outputKey), new Text(outputValues))`;
	    }
        }
    }
